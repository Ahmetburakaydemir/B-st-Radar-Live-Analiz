...
try:
    ...
    st.error("⚠️ API Anahtarı bulunamadı! Lütfen Streamlit panelinden 'Secrets' ayarını kontrol et.")
    st.stop()
...
def yapay_zeka_yorumu_al(sembol, fiyat, fk, pd_dd, rsi, degisim):
    """Google Gemini-Pro modeline verileri gönderip yorum alır"""
    try:
        # BURAYI DEĞİŞTİRDİK: En garanti çalışan model 'gemini-pro'dur.
        ...
        Yatırım tavsiyesi (AL/SAT) verme, sadece verilerin ne anlama geldiğini yorumla.
        
        VERİLER:
        - Hisse Kodu: {sembol}
        - Anlık Fiyat: {fiyat} TL
        - Günlük Değişim: %{degisim:.2f}
        - F/K Oranı: {fk} (Sektör ortalaması genelde 8-10 arasıdır)
        - PD/DD Oranı: {pd_dd}
        - RSI Değeri: {rsi:.1f} (30 altı ucuz, 70 üstü pahalı sinyali sayılır)
        """
        
        response = model.generate_content(prompt)
        return response.text
    except Exception as e:
        return f"AI Yorumu alınırken bir hata oluştu: {e}"
    ...
    try:
        with st.spinner(f'{sembol} taranıyor ve Yapay Zeka raporu hazırlanıyor...'):
            ...
            if 'currentPrice' not in bilgi:
                st.error("❌ Veri çekilemedi. Hisse kodunu yanlış girmiş olabilirsin.")
            else:
                # Hesaplamalar
                ...
                # --- GRAFİK ---
                st.subheader("Teknik Görünüm (Mum Grafiği)")
                fig = go.Figure()
                fig.add_trace(go.Candlestick(x=hist.index, open=hist['Open'], high=hist['High'],
                                             low=hist['Low'], close=hist['Close'], name='Fiyat'))
                fig.update_layout(height=400, template="plotly_dark", title=f"{sembol} Analiz Grafiği")
